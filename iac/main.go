...
func main() {
	pulumi.Run(func(ctx *pulumi.Context) error {
		...
		managementProvider, err := aws.NewProvider(ctx, "management-provider", &aws.ProviderArgs{
			Profile: pulumi.String("management"),
		...
		appsProvider, err := aws.NewProvider(ctx, "prod-provider", &aws.ProviderArgs{
			Profile: pulumi.String("apps"),
		...
		for _, distributionConfig := range distributionConfigs {
			distribution, err := cloudfront.NewDistribution(ctx, distributionConfig.dns, &cloudfront.DistributionArgs{
				DistributionConfig: &cloudfront.DistributionConfigArgs{
					...
					Origins: &cloudfront.DistributionOriginArray{
						...
						&cloudfront.DistributionOriginArgs{
							...
							OriginPath:            pulumi.String("/static/ipv6test.app"),
							Id:                    pulumi.String("S3"),
				...
				Tags: commonTags,
			}, pulumi.Provider(appsProvider), pulumi.IgnoreChanges([]string{"tags"}))
			if err != nil {
				return err
			}
